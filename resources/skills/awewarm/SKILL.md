...
| Hub admin — moved to the separate `awewarm-hub` package ([wehuman01/awewarm-hub](https://github.com/wehuman01/awewarm-hub), `pip install awewarm-hub`; operator skill at `resources/skills/awewarm-hub/SKILL.md` there) | The operator runs `awewarm-hub serve` and `awewarm-hub status / invite / list users / list invites / invite revoke|restore|rename / config` there (same `--data-dir`, default `~/.awewarm-server`; bare `invite` mints, `invite revoke|restore|rename` operate on a code). `awewarm serve --hub` and `awewarm hub ...` now die with a tombstone naming the replacement. Hub *users* still use this package: `awewarm remote connect <url> --invite awi_...`. |
...
| "Keep it warm but I have no always-on machine", "没有常开机器也想保温" | The developer's community hub at https://awewarm.wehuman.top — request an invite at peng@wehuman.top, then pair with `awewarm remote connect <url> --invite awi_...` and delegate as usual. State the trust rule first (the user's API keys pass through the hub's RAM). User tutorial: https://github.com/wehuman01/awewarm/blob/main/docs/community-hub/README.md |
| "Share one server with my team", "多人共用一台服务器/hub" | Operator installs the separate `awewarm-hub` package on the box (`awewarm-hub serve`, hands out `awewarm-hub invite` codes); each user runs `awewarm remote connect <url> --invite awi_...`. Warn: hub users must trust the box's operator/root — their API keys pass through its RAM. |
...
| "Warm my two codex/claude accounts separately", "多个codex账号分别保温" | `awewarm discover` lists each aweswitch account as its own entry (`Codex (cxo-peng)`); add each with `awewarm config add` — each connection carries its own `authHome`, so local fires and delegated pushes stay per account. Set schedules per connection with `awewarm config set`. A claude aweswitch account only appears once it has a file login (`.credentials.json`). |
| "status says server unreachable but warm-ups work", "代理把 awewarm 劫持了" | By design: awewarm ignores http_proxy/https_proxy/all_proxy and egresses direct; the error names the egress used. If the network truly needs a proxy: `awewarm config proxy <url>` (show: no args; clear: `none`) — it routes awewarm's own traffic and the codex/claude subprocesses it starts; unset, the CLIs are unaffected. |
