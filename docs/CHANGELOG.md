...
### Tuning knobs grouped under `settings` in config.json

The catch-up/degrade knobs (`catchupMinutes`, `catchupAttempts`, `degradeAfterNodes`) move from the top level of each connection into a `settings` object, so tuning sits in one place instead of interleaving with identity and schedule fields. The keys keep their names; defaults are still omitted on save (a connection running all-default knobs gets no `settings` at all). Files written by earlier v0.3.6 builds with the knobs flat migrate into `settings` in place on first load.
