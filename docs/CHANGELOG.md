...
The on-disk format stays v3: older spellings — a top-level `windowMinutes` on a connection, a knob-position `windowMinutes`, a schedule-position `wakeWhenAsleep` — fold into the current positions on load and are never written again, so files migrate themselves on their next save. Runtime behavior for existing configs is unchanged (window, schedule, and catch-up values round-trip identically; the resolved runtime shape the wake layer, status, and delegation pushes read is byte-identical).
